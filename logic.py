...
def shuffle_meals(pool, carryover=None, existing=None):
    """Return a dict {day: meal} for all 7 days.
...
def shuffle_tasks(task_names, existing=None):
    """Randomly distribute a list of task names across the 7 days.
    Returns {day: [task names]}. Preserves any placements already in
    `existing`."""
    result = {d: list(v) for d, v in (existing or {}).items()}
    for d in DAYS:
        result.setdefault(d, [])
